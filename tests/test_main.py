...
def test_update_virtual_tag(vantage_sdk, virtual_tag_fixture):
    ...
    new_value = UpdateVirtualTagConfigValue(
        filter="costs.provider = 'aws' AND costs.service = 'Amazon Simple Storage Service'", name=new_value_name
    )
...
@pytest.mark.skip(reason="Access Grant tests require Team creation which needs special enterprise permissions")
...
@pytest.mark.skip(reason="Team creation requires special enterprise permissions")
...
def test_update_team(vantage_sdk, team_fixture):
    ...
    team_update = UpdateTeam(name="updated_team_name", description="Updated description")
    updated_team = vantage_sdk.update_team(params, team_update)

    assert updated_team is not None
    assert updated_team.name == "updated_team_name"
...
def test_get_me(vantage_sdk):
    ...
    assert hasattr(me_info, "workspaces")
    assert hasattr(me_info, "bearer_token")

    # Check bearer token attributes
    if me_info.bearer_token is not None:
        assert hasattr(me_info.bearer_token, "description")
        assert hasattr(me_info.bearer_token, "created_at")
        assert hasattr(me_info.bearer_token, "scope")


# ---- Anomaly Alerts & Notifications Tests ----


@pytest.mark.skip(reason="Anomaly alerts can only be created via the UI")
def test_get_all_anomaly_alerts(vantage_sdk):
    ...
    anomaly_alerts = vantage_sdk.get_all_anomaly_alerts(params)
    assert anomaly_alerts is not None
...
@pytest.mark.skip(reason="Billing rules require special enterprise permissions")
...
def test_update_billing_rule(vantage_sdk, billing_rule_fixture):
    ...
    # Update the billing rule title
    billing_rule_update = UpdateBillingRule(title="updated_billing_rule_title")
    updated_rule = vantage_sdk.update_billing_rule(params, billing_rule_update)

    assert updated_rule is not None
    assert updated_rule.title == "updated_billing_rule_title"
...
def test_update_budget(vantage_sdk, budget_fixture):
    ...
    budget_update = UpdateBudget(name="updated_budget_name")
    updated_budget = vantage_sdk.update_budget(params, budget_update)

    assert updated_budget is not None
    assert updated_budget.name == "updated_budget_name"
...
def test_update_cost_alert(vantage_sdk, cost_alert_fixture):
    ...
    # Update the cost alert threshold and title
    cost_alert_update = UpdateCostAlert(
        threshold=5000,  # Update to alert at $5000
        title="updated_cost_alert_title",
    )
    ...
    assert updated_cost_alert.title == "updated_cost_alert_title"
...
def test_update_dashboard(vantage_sdk, dashboard_fixture):
    params = DashboardTokenParams(dashboard_token=dashboard_fixture.token)
    updated_title = f"Updated_{dashboard_fixture.title}"
...
def test_update_financial_commitment_report(vantage_sdk, financial_commitment_report_fixture):
    updated_title = f"Updated_{financial_commitment_report_fixture.title}"
...
def test_update_kubernetes_efficiency_report(vantage_sdk, kubernetes_efficiency_report_fixture):
    updated_title = f"Updated_{kubernetes_efficiency_report_fixture.title}"
...
@pytest.mark.skip(reason="Managed accounts require special enterprise permissions")
...
def test_update_managed_account(vantage_sdk, managed_account_fixture):
    updated_name = f"Updated_{managed_account_fixture.name}"
...
def test_update_network_flow_report(vantage_sdk, network_flow_report_fixture):
    updated_title = f"Updated_{network_flow_report_fixture.title}"
...
def test_update_resource_report(vantage_sdk, resource_report_fixture):
    updated_title = f"Updated_{resource_report_fixture.title}"
...
@pytest.mark.skip(reason="This test simply hangs indefinitely")
def test_get_all_resources(vantage_sdk, resource_report_fixture):
    query_params = ResourcesGetParametersQuery(resource_report_token=resource_report_fixture.token)
    resources = vantage_sdk.get_all_resources(query_params)
    assert resources is not None


@pytest.mark.skip(reason="This test requires a specific resource setup")
def test_get_resource(vantage_sdk, resource_report_fixture):
    params = ResourceTokenParams(resource_token=resource_report_fixture.token)
    resource = vantage_sdk.get_resource(params)
    assert resource is not None
    assert resource.token == resource_report_fixture.token
...
def test_update_tags(vantage_sdk, virtual_tag_fixture):
    # First we need to get the tag representation from the virtual tag
    tags = vantage_sdk.get_all_tags()
    tag = next((t for t in tags.tags if t.tag_key == virtual_tag_fixture.key), None)

    # Get the key and current hidden state
    tag_key = tag.tag_key
    current_hidden = tag.hidden

    # Toggle its hidden state
    tag_update = UpdateTag(tag_key=tag_key, hidden=not current_hidden)

    updated_tag = vantage_sdk.update_tags(tag_update)
    assert updated_tag is not None
    assert updated_tag.tag_key == tag_key
...
def test_get_all_unit_costs(vantage_sdk, cost_report_fixture):
    ...
    assert hasattr(unit_costs, "unit_costs")


@pytest.mark.skip(reason="Requires actual cost data to export and needs more specific test setup")
def test_create_unit_costs_data_export(vantage_sdk):
    # This would need a specific test setup to create actual cost data to export
...
@pytest.mark.skip(reason="Creates actual user feedback data which we don't want to do in automated tests")
...
@pytest.mark.skip(reason="Deleting workspaces returns 500 Internal Server Error")
def test_update_workspace(vantage_sdk, workspace_fixture):
    """Test updating a workspace's name"""
    updated_name = f"Updated_{workspace_fixture.name}"
